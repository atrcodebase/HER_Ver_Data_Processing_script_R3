      ...
      expr = {					
        for (rowi in 1:nrow(log)) {
          ...
          if (var_i %in% colnames(data)) {
            ...
            if (all(var_type %in% c("POSIXct", "POSIXt")) & is.na(new_i)){ # Newly Added
...
# Relevancy Function -------------------------------------------------------------------------------
check_relevancy_rules <- function(data, tool_relevancy, sheet_name, KEY="KEY", print=TRUE){
  # initiate Log
  `%notin%` <- Negate(`%in%`)
...
update_series_cols <- function(data, tool_path, question_separator, excluded_col=""){
  ...
  for(question in sm_cols){
    ...
    series_cols <- names(data)[grepl(paste0("^",question, question_separator, "[0-9]{1,5}$"), names(data))] # Regex: detect the question ended by 1 to 4 numbers followed by nothing else
...
update_media_links_1 <- function(data, link_cols, download_link, key_col="KEY", rename=FALSE){
...
update_media_links <- function(data, tool_path, download_link="https://artftpm.surveycto.com/view/submission-attachment/", key_col="KEY", rename=FALSE){
  ...
  # Add URL links to each file
  data <- update_media_links_1(data, link_cols, download_link, key_col, rename)
...
labeler <-function (data, tool, survey_label = "label::English", choice_lable = "label::English", 
                    multi_response_sep = ";", excluded_cols="") 
{
  ...
  # New: Exclude specific questions (to be labeled manually)
  survey_questions <- survey_questions %>% filter(name %notin% excluded_cols)
  #
...
log_questions <- function(data, columns, columns_different="", key_col="KEY", suffix, sheet){
  ...
  for(col in columns){
    ...
    if(calculated_col %notin% names(data)){
      ...
      if(paste0(str_remove(col, "_audio$"), "_", suffix) %in% names(data)){ # New condition
        calculated_col <- paste0(str_remove(col, "_audio$"), "_", suffix)
      } else if(paste0(str_remove(col, "_Photo$"), "_", suffix) %in% names(data)){ # New condition
        calculated_col <- paste0(str_remove(col, "_Photo$"), "_", suffix)
      } else {
    ...
    if(suffix %in% c("QA", "qa_status", "qa")){ # New
...
check_responses <- function(data, tool_path, sheet, excluded_cols="", print=TRUE){
  # Read tool
...
# Reshape tool for select multiple questions
reshape_tool <- function(tool_path, excluded_cols=""){
  ...
  tool_choices <- tool_choices %>%
    filter(!is.na(list_name) & !(list_name %in% label & list_name %in% label)) %>%
    select(list_name, response_code=value, response_label=label) %>%
    mutate(response_label=str_squish(response_label),
...
# Apply new labels for Select Multiple series columns 
apply_SM_Label <- function(client_dt, tool, excluded_cols=""){
...
# Add escape character before each character
escape_characters <- function(text, new_characters="") {
  # Typical characters that need to be escaped
  characters_to_escape <- c("/", "(", ",", ".", ")", "’", "–", "[", "^", "$", 
                            "?", "*", "+", "}") # , new_characters
  # Escape each character
  for (char in characters_to_escape) {
    text <- str_replace_all(text, fixed(char), paste0("\\", char))
  }
  return(text)
}

# Read all excel sheets and add them to a list
read_xlsx_sheets = function(path){
  guess_max <- 5000000
  convert_to_na <- c("N/A", "-", " ") # values to convert to NA
  df <- purrr::map2(path, getSheetNames(path), ~ readxl::read_excel(.x, sheet = .y, guess_max=guess_max, na=convert_to_na), .progress = TRUE) |> setNames(getSheetNames(path)) # , .name_repair = "minimal"
  if (length(df) == 1) df <- data.frame(unname(df))
  return(df)
} 

#### Convert audio translation columns to character
convert_to_char <- function(data, tool_path, image_suffix, audio_suffix){
  # Read tool
  media_cols <- read_excel(tool_path, "survey", guess_max = 100000)
  
  media_cols <- media_cols %>%
    filter(type %in% c("audio", "image") & name %in% names(data)) %>%
    mutate(name= case_when(
      type %in% "image" ~ paste0(name, "_", image_suffix),
      # type %in% "audio" ~ paste0(name,"_", audio_suffix),
      type %in% "audio" & paste0(name,"_", audio_suffix) %in% names(data) ~ paste0(name,"_", audio_suffix),
      type %in% "audio" & paste0(str_remove(name, "_audio$"),"_", audio_suffix) %in% names(data) ~ paste0(str_remove(name, "_audio$"),"_", audio_suffix),
      TRUE ~ name
    )) %>% pull(name)
  
  # Change to Character
  data <- data %>% mutate(across(any_of(media_cols), as.character))
  return(data)
}
...
####  Flag Correction log issues
check_log <- function(correct_log, data, Log_Tool_name, data_KEY="KEY"){
  # Assign sheets
  if(!is.data.frame(data)){data_sheets=names(data)}else{data_sheets="data"}
  
  for(sheet in data_sheets){
    correct_log <- correct_log %>% 
      mutate(issue = case_when(
        # List Data
        is.data.frame(data) == FALSE & Tool == Log_Tool_name & Tab_Name %in% sheet & question %notin% names(data[[sheet]]) ~ "question",
        is.data.frame(data) == FALSE & Tool == Log_Tool_name & Tab_Name %in% sheet & KEY %notin% data[[sheet]][[data_KEY]] ~ "KEY",
        # # Single sheet
        is.data.frame(data) == TRUE & Tool == Log_Tool_name & Tab_Name %in% sheet & question %notin% names(data) ~ "question",
        is.data.frame(data) == TRUE & Tool == Log_Tool_name & Tab_Name %in% sheet & KEY %notin% data[[data_KEY]] ~ "KEY",
        TRUE ~ issue
      )) 
  }
